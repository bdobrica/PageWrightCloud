...
- [ ] M2.4 Package `pagewrightc` and a trusted versioned starter theme. After edits, validate allowed source changes, compile into a fresh output directory before replacing workspace `public/`, check HTML/assets and produce truthful manifest results.
...
- [ ] M2.9 Persist Redis data and durable job/version history; reconcile pending/running/uncertain jobs after restart, including a crash between gateway dispatch claim and manager receipt. M1.2 reservations no longer expire; legacy TTLs remain. Add bounded retention without expiring deduplication while execution is possible, and a safe operator/automatic recovery policy when manager evidence is missing.
