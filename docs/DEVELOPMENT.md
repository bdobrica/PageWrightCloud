...
available to that environment. Run commands from the repository root. Complete
the [root secret setup](../README.md#local-setup) before Compose commands.
...
so BuildKit is required. The production worker includes the real pinned CLI,
trusted compiler and theme. A build alone does not prove provider access;
use the isolated acceptance procedures in [testing](TESTING.md).
...
Serving coverage includes compiled artifacts and real Nginx activation/rollback.
Separate Docker and installed-CLI checks exercise actual worker launch/sandboxing.
Gateway tests use a private schema.
Direct tagged tests require explicit test database/service URLs; prefer the harness.

See [TESTING.md](TESTING.md) for CI, opt-in tests and remaining boundaries. M4.12
resolved the two remaining serving skips; local results are not hosted CI.
...
without inventing missing job mappings. See [submission/retry semantics](adr/0002-architecture-decisions-build-submissions.md)
...
existing installations' artifacts. Container storage uses `/nfs`. Only UI, gateway
and public edge ports are published; internal port overrides no longer expose APIs
or databases. Container names are scoped by Compose project. For local hosting,
configure matching live/preview DNS and separate app origins in a local namespace;
the disposable browser harness supplies its own localhost fixtures.
...
the separate browser journey covers editing, compilation and publishing.
