...
func assertSchema(t *testing.T, db *DB) {
	t.Helper()
	var versions int
	if err := db.Get(&versions, "SELECT count(*) FROM schema_migrations"); err != nil || versions != 6 {
		t.Fatalf("migration count = %d, error = %v", versions, err)
	}
	for _, table := range []string{"users", "sites", "site_aliases", "versions", "password_reset_tokens"} {
		var exists bool
