// Disposable full-production-stack acceptance. All application writes are UI actions.
...
const env = {
  ...
  PAGEWRIGHT_WORKER_LLM_KEY: 'browser-acceptance-dummy-key', PAGEWRIGHT_WORKER_LLM_URL: 'http://fixture:8090/v1',
  PAGEWRIGHT_WORKER_LLM_MODEL: 'acceptance-fixture', PAGEWRIGHT_WORKER_IMAGE: `${project}-worker:acceptance`,
  PAGEWRIGHT_WORKER_TIMEOUT: '5m', PAGEWRIGHT_DISPATCH_CONCURRENCY: '1',
...
function docker(args, capture = false, required = true) {
  const result = spawnSync('docker', args, { env, encoding: 'utf8', stdio: capture ? 'pipe' : 'inherit', timeout: 900_000 });
