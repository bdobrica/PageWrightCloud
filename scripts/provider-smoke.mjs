...
// Reject remote contexts before reading a credential or creating any resources.
const contextHost = execFileSync('docker', ['context', 'inspect', '--format', '{{.Endpoints.docker.Host}}'], {
  env: { PATH: process.env.PATH, HOME: process.env.HOME, DOCKER_CONFIG: process.env.DOCKER_CONFIG },
  encoding: 'utf8', timeout: 10000,
}).trim();
assert.ok(contextHost.startsWith('unix://'), 'local Docker daemon required');
...
const env = { PATH: process.env.PATH, HOME: process.env.HOME, DOCKER_CONFIG: process.env.DOCKER_CONFIG,
  ...
  SMOKE_SERVICE_TOKEN: serviceToken, SMOKE_WORKER_IMAGE: `${project}-worker:acceptance`,
...
const request = async (url, options = {}, status = 200) => {
  ...
  if (serviceOrigins.has(new URL(url).origin)) headers.set('authorization', `Bearer ${serviceToken}`);
...
try {
  // Always exercise this checkout, never a historical worker tag left on host.
  docker(['build', '--target', 'production', '-t', env.SMOKE_WORKER_IMAGE, '-f', 'pagewright/worker/Dockerfile', '.'], { stdio: 'inherit' });
  compose('up', '-d', '--build', '--wait', '--wait-timeout', '120');
  const manager = endpoint('manager', 8081), storage = endpoint('storage', 8080), budget = endpoint('budget', 8090);
  serviceOrigins.add(new URL(manager).origin);
  serviceOrigins.add(new URL(storage).origin);
  ...
  assert.ok(!inspected.Config.Env.some(entry => entry === `PAGEWRIGHT_SERVICE_TOKEN=${serviceToken}`));
  assert.ok(inspected.Config.Env.some(entry => entry.startsWith('PAGEWRIGHT_WORKER_TOKEN=worker-v1.')));
